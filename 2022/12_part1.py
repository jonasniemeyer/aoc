def fewest_steps(input_str: str) -> int:
    ...
    start = [(row, col) for row in range(len(heightmap)) for col in range(len(heightmap[0])) if heightmap[row][col] == "S"][0]
    heightmap[start[0]][start[1]] = "a"
    destination = [(row, col) for row in range(len(heightmap)) for col in range(len(heightmap[0])) if heightmap[row][col] == "E"][0]
    heightmap[destination[0]][destination[1]] = "z"
