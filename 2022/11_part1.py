def monkey_business(input_str: str) -> int:
    ...
    for part in parts:
        ...
        if "old * old" in lines[2]:
            operation = lambda x=0: (x * x) // 3
        elif "old * " in lines[2]:
            order = int(lines[2].split("old * ")[1])
            operation = lambda x=0, order=order: (x * order) // 3
        elif "old + " in lines[2]:
            order = int(lines[2].split("old + ")[1])
            operation = lambda x=0, order=order: (x + order) // 3
    ...
    for _ in range(ROUNDS):
        for monkey in items.keys():
            for _ in range(len(items[monkey])):
                ...
                new_item = operations[monkey](items[monkey].pop(0))
                target_monkey = targets[monkey](new_item)
                items[target_monkey].append(new_item)
